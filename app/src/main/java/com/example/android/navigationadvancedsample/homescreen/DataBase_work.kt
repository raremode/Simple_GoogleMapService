...
class DataBase_work(context: Context?) {
    ...
    fun insert(LATITUDE: String?, longitude: String?) { //функция заполнения БД точек сбора мусора
        cv.put(DataBase.LATITUDE, LATITUDE)
        cv.put(DataBase.LONGITUDE, longitude)
        db.insert(DataBase.TYPEFILTER, null, cv)
    }

    fun rec_x(): ArrayList<String> { //получение переменных широты
        val alist = ArrayList<String>()
        cursor = db.query(DataBase.TYPEFILTER, null, null, null, null, null, null)
    ...
    fun rec_y(): ArrayList<String> { //получение переменных долготы
        ...
        cursor2 = db.query(DataBase.TYPEFILTER, null, null, null, null, null, null)
